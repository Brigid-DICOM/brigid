...
async function getDicomStats(workspaceId: string, range?: string) {
    ...
    const patientCount = await patientService.getPatientCount(workspaceId, range);
    const instanceService = new InstanceService();
    const instanceCount = await instanceService.getInstanceCount(workspaceId, range);
    const seriesService = new SeriesService();
    const modalityCount = await seriesService.getUniqueModalities(workspaceId, range);
