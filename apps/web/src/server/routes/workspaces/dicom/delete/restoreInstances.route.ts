...
import {
    restoreInstancesBodySchema
} from "@/server/schemas/dicomDelete";
import {
    DicomDeleteService
} from "@/server/services/dicom/dicomDelete.service";
import { InstanceService } from "@/server/services/instance.service";

const restoreInstancesRoute = new Hono().post(
    "/workspaces/:workspaceId/dicom/instances/restore",
    describeRoute({
        description: "Restore DICOM instances",
        tags: ["DICOM"]
    }),
    ...
    zValidator("json", restoreInstancesBodySchema),
    async (c) => {
        ...
        const localInstanceIds = await instanceService.getInstancesBySopInstanceUid(workspaceId, distinctInstanceIds);
        ...
        const result = await deleteService.restoreInstances(
            workspaceId,
...
export default restoreInstancesRoute;