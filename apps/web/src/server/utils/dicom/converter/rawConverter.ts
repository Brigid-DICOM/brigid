...
import tmp from "tmp";
import type { ConvertOptions,  ConvertResult,  DicomSource, OutputFormat } from "@/server/types/dicom/convert";
...
export class RawConverter implements DicomToImageConverter {
        ...
        if (source.kind !== "stream") {
            ...
            if (numberOfFrames > 1 && !("frameNumber" in options)) {
                for (let i = 1; i <= numberOfFrames; i++) {
                    ...
                    await this.generateRawFrameStream(
                        tmpFile.name,
                        destFile,
                        i,
                    );
    ...
    private async generateRawFrameStream(source: string, dest:string, frameNumber: number): Promise<void> {
