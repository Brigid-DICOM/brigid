...
export function toConvertOptions(qs: Record<string, string>): ConvertOptions {
    ...
    const windowWidth = parsed.window?.split(",")?.at(1)
        ? Number(parsed.window?.split(",")?.at(1))
        : undefined;
    const windowCenter = parsed.window?.split(",")?.at(0)
        ? Number(parsed.window?.split(",")?.at(0))
        : undefined;
    ...
    return {
        ...
        windowWidth,
        windowCenter,
        voiLutFunction: parsed.window?.split(",")?.at(2) as
            | "linear"
            | "linear-exact"
            | "sigmoid"
            | undefined,
...
export function wadoUriParamsToConvertOptions(
