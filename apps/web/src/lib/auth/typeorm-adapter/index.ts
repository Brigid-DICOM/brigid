...
export async function getManager(options: {
    ...
    // 在我們的專案中，都會執行 migration，這裡應該不影響
...
export function TypeORMAdapter(
    ...
    const c = {
        ...
        entities: {
            ...
            SessionEntity:
                entities?.SessionEntity ?? defaultEntities.SessionEntity,
            AccountEntity:
                entities?.AccountEntity ?? defaultEntities.AccountEntity,
            VerificationTokenEntity:
                entities?.VerificationTokenEntity ??
                defaultEntities.VerificationTokenEntity,
            // 加入其他的 entities 避免找不到 relation metadata
            ...entities,
        },
    };
    ...
    return {
        ...
        async getUserByAccount(provider_providerAccountId) {
            ...
            const account = await m.findOne<
                AdapterAccount & { user: AdapterUser }
            >(
                AccountEntityName,
                // @ts-expect-error
        ...
        async createVerificationToken(data) {
            const m = await getManager(c);
            const verificationToken = await m.save(
                VerificationTokenEntityName,
                data,
            );
            // @ts-expect-error
