...
export function DateRangePicker({ value, onChange, className }: DateRangePickerProps) {
  // 解析當前值來決定類型和日期
  const parseValue = (val: string): { type: DateRangeType; fromDate: string; toDate: string } => {
  ...
  // 更新最終值
  const updateValue = React.useCallback((newType: DateRangeType, newFrom: string, newTo: string) => {
    let result = "";
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <div className="flex items-center space-x-2">
        {(rangeType === "exact" || rangeType === "from" || rangeType === "range") && (
          <DatePicker
            value={fromValue}
