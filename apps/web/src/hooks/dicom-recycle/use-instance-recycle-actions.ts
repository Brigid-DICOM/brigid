...
import { deleteDicomInstanceMutation, recycleDicomInstanceMutation, restoreDicomInstanceMutation } from "@/react-query/queries/dicomInstance";
...
export function useInstanceRecycleActions({ workspaceId, studyInstanceUid, seriesInstanceUid }: UseInstanceRecycleActionsProps) {
    ...
    const recycleInstance = useMutation({
        ...
        onMutate: (_, context) => {
            toast.loading(t("dicom.messages.recycling", { level: "instance" }), {
                id: context.meta?.toastId as string,
            });
        },
        onSuccess: (_, __, ___, context) => {
            toast.success(t("dicom.messages.recycleSuccess", { level: "instance" }));
            toast.dismiss(context.meta?.toastId as string);
            invalidate();
            clearSelection();
        },
        onError: (_, __, ___, context) => {
            toast.error(t("dicom.messages.recycleError", { level: "instance" }));
            toast.dismiss(context.meta?.toastId as string);
            clearSelection();
        },
    });
