...
import { type Protocol, protocol, type Session } from "electron";
...
export function createHandler({
  ...
  const handleRequest = async (
    ...
    try {
      ...
      if (url.pathname.startsWith(LOCAL_STORAGE_URL_PREFIX + "/")) {
        if (debug)
          logger.debug(`Skipping local file service request: ${request.url}`);
      ...
      if (isDev) {
        // 开发环境：转发请求到开发服务器
        if (debug)
          logger.debug(`Forwarding request to dev server: ${request.url}`);
        ...
        if (debug)
          logger.debug(`Processing with Next.js handler: ${request.url}`);
      ...
      if (debug)
        logger.debug(
          `Request processed: ${request.url}, status: ${response.status}`
        );
      return response;
    } catch (e) {
  ...
  const createInterceptor = ({ session }: { session: Session }) => {
    ...
    logger.debug(
      `Creating interceptor with session in ${
    ...
    if (!registerProtocolHandle) {
      logger.debug(
        `Registering HTTP protocol handler in ${
          isDev ? "development" : "production"
        } mode`
      );
      protocol.handle("http", async (request) => {
        if (!isDev) {
          ...
          const isLocalFileService = url.pathname.startsWith(
    ...
    return function stopIntercept() {
      ...
      logger.debug(
        `Stopping interceptor, remaining count: ${interceptorCount}`
      );

      // 只有当没有活跃的拦截器时才取消注册协议处理器
      if (registerProtocolHandle && interceptorCount === 0) {
        logger.debug(
          "Unregistering HTTP protocol handler (no active interceptors)"
        );
