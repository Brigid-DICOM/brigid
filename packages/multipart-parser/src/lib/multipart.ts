/**
 * Original from https://github.com/remix-run/remix/tree/main/packages/multipart-parser
 */

import { randomBytes } from 'node:crypto'
import { createWriteStream, readFileSync, statSync, unlink, type WriteStream } from 'node:fs'
import { tmpdir } from 'node:os'
import path from 'node:path'
import Headers from '@remix-run/headers'
import type { PartialTailSearchFunction, SearchFunction } from './buffer-search.ts'
import { createPartialTailSearch, createSearch } from './buffer-search.ts'
...
export interface ParseMultipartOptions {
  ...
  /**  
   * The directory where temporary files should be written. If not specified,  
...
export class MultipartParser {
  ...
  #currentFileHandle: WriteStream | null = null
  ...
  async *write(chunk: Uint8Array): AsyncGenerator<MultipartPart, void, unknown> {
    ...
    while (true) {
      ...
      if (this.#state === MultipartParserStateHeader) {
        ...
        this.#currentTempFile = path.join(  
          this.uploadDir,  
          `upload-${randomBytes(16).toString('hex')}.tmp`  
        )  
        this.#currentFileHandle = createWriteStream(this.#currentTempFile)  

        this.#currentPart = new MultipartPart(chunk.subarray(index, headerEndIndex), [], this.#currentTempFile)
  ...
  async #append(chunk: Uint8Array): Promise<void> {
    if (this.#contentLength + chunk.length > this.maxFileSize) {
      if (this.#currentFileHandle) {
        await new Promise<void>((resolve) => {  
          this.#currentFileHandle?.end(resolve)  
        })  
    ...
    if (this.#currentFileHandle) {
      const canContinue = this.#currentFileHandle.write(chunk)  
      if (!canContinue) {  
        // Wait for drain event if buffer is full  
        await new Promise<void>((resolve) => {  
          this.#currentFileHandle?.once('drain', resolve)  
        })  
      }  
    }
...
export class MultipartPart {
  ...
  readonly content: Uint8Array[]
    ...
    /**  
   * The path to the temporary file containing this part's content.  
   * This will be set if uploadDir was specified in the parser options.  
   */  
  ...
  constructor(header: Uint8Array, content: Uint8Array[], tempFilePath: string | null = null) {  
    this.#header = header  
  ...
  get bytes(): Uint8Array {
    ...
    // If we have a temp file, read from it  
    if (this.tempFilePath) {  
      this.#cachedBytes = readFileSync(this.tempFilePath)  
      return this.#cachedBytes  
    }  
  
    // Otherwise, concatenate the content chunks  
  ...
  get size(): number {
    if (this.tempFilePath) {  
      try {  
        return statSync(this.tempFilePath).size  
      } catch {  
        return 0  
      }  
    }
